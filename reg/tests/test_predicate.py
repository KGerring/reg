...
def test_multi_predicate_get_key():
    ...
    p = PredicateRegistry(match_key("a", a_key), match_key("b", b_key))
...
def test_multi_predicate_fallback():
    r = PredicateRegistry(match_key("a", fallback="fallback1"), match_key("b", fallback="fallback2"))
...
def test_name_request_method_body_model_registered_for_base():
    m = PredicateRegistry(match_key("name", fallback="name fallback"), match_key("request_method", fallback="request_method fallback"), match_instance("body_model", fallback="body_model fallback"))
...
def test_name_request_method_body_model_registered_for_base_and_sub():
    m = PredicateRegistry(match_key("name", fallback="name fallback"), match_key("request", fallback="request_method fallback"), match_instance("body_model", fallback="body_model fallback"))
...
def test_multi_key_by_predicate_name():
    p = PredicateRegistry(match_key("foo", default="default foo"), match_key("bar", default="default bar"))
    assert p.key_dict_to_predicate_key({"foo": "FOO", "bar": "BAR"}) == ("FOO", "BAR")
