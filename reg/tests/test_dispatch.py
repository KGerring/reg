...
def test_all():
    ...
    assert target.by_args(sub).all_matches == [registered_for_sub, registered_for_base]
...
def test_all_by_keys():
    ...
    assert target.by_predicates(obj=Sub).all_matches == [registered_for_sub, registered_for_base]
...
def test_add_predicates_no_defaults():
    ...
    view.add_predicates([match_instance("model", get_model, model_fallback), match_key("name", get_name, name_fallback), match_key("request_method", get_request_method, request_method_fallback)])
...
def test_dispatch_predicates_register_defaults():
    ...
    view.add_predicates(
        [
            ...
            match_key("request_method", get_request_method, request_method_fallback, default="GET"),
        ]
    )
...
def test_key_dict_to_predicate_key():
    @dispatch(match_key("foo", default="default foo"), match_key("bar", default="default bar"))
    def view(self, request):
        raise NotImplementedError()
...
def test_key_dict_to_predicate_key_unknown_keys():
    ...
    assert view.by_predicates(unknown="blah").key == ("default foo", "default bar")
...
def test_fallback_should_already_use_subset():
    ...
    view.register(collection_add, model=Collection, request_method="POST", body_model=Item)
